    ...
    public static void main(String[] Args){
        ...
        for (int i = 0; i < firstSplited.size(); i++) {
            ...
            for (int j = 0; j < steps; j++){
                ...
                lastX = p.getX();
                lastY = p.getY();
                path1.put(p.toString(), stepsum);
            }
        }
        lastX = lastY = 0;
        stepsum = 0;
        for (int i = 0; i < secondSplited.size(); i++) {
            steps= Integer.parseInt(secondSplited.get(i).replaceAll("[^0-9]", ""));
            direction = secondSplited.get(i).substring(0,1);
            ...
            for (int j = 0; j < steps; j++){
                ...
                for (int k = 0; k < path1.size(); k++){
                    if (path1.containsKey(p.toString()) && (stepsum + path1.get(p.toString())) < fewestCombinedSteps){
                        fewestCombinedSteps = stepsum + path1.get(p.toString());
                    }
                }
            }
        }
        System.out.println("Fewest combined steps: " +fewestCombinedSteps); //101386
    }
}
